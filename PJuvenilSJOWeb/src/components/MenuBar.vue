...
const visibleLoginBtn = ref(true);
const visibleLogoutBtn = ref(false);
const password = ref();
const userName = ref();
...
onMounted(() => {
  if (localStorage.getItem("token") != null) {
    visibleLoginBtn.value = false;
    visibleLogoutBtn.value = true;
  }
});
...
const iniciarSesion = async () => {
  ...
  if (login.error) {
    ...
    localStorage.setItem("token", login.data.session.access_token);
    console.log(login.data.session.access_token);
